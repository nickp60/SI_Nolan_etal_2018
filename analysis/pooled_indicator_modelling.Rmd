...
```{r, results='hide'}
coliform_fit = stan(file = "../models/pooled_indicator_LP_NW.stan",
                    data = coliform_trt_stan_data,
                    iter = 8000,
                    chains = 4)
coliform_fit_lp = stan(file = "../models/pooled_indicator_LP.stan",
                    data = coliform_trt_stan_data,
                    iter = 8000,
                    chains = 4)
```
...
basic_fit_plots(ecoli_fit, condition_names = ecoli_data$treatment, ignored_params = c("lp_", "y_hat"))
```
...
```{r, results='hide'}
enterococci_fit = stan(file = "../models/pooled_indicator_LP_NW.stan",
                       data = enterococci_trt_stan_data,
                       iter = 4000,
                       chains = 4)
enterococci_fit_lp = stan(file = "../models/pooled_indicator_LP.stan",
                       data = enterococci_trt_stan_data,
                       iter = 4000,
                       chains = 4)
```
