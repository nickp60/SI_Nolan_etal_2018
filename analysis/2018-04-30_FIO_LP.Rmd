---
title: "Piecewise Linear Fit of FIO Data"
...
To analyse the FIO levels under the effect of various treatments (10L/50mL vessels; pasteurised/unpasteurised; pre-pasteurisation; post-pasteurisation) we use piecewise linear regression to estimate the rate of FIO loss at two stages of fermentation: "early" and "late".
...
- The feedstock (`FS`) data are removed:
...
```{r}
summary_coliform = summary(coliform_fit, pars=c("m1", "c1", "m2", "c2"), probs = c(0.25, 0.75))
...
```{r}
summary_ecoli = summary(ecoli_fit, pars=c("m1", "c1", "m2", "c2"), probs = c(0.25, 0.75))
...
```{r}
summary_enterococci = summary(enterococci_fit, pars=c("m1", "c1", "m2", "c2"), probs = c(0.25, 0.75))
